...
function [qmax,Imax] = Find_Peak_for_hwhm( Data, q_range, mode)
...
j_max=min(j, size(v,1));

% Fix the ends.
if (size(v,1)>(j_min+1)) integral=integral+ ( 0.5*( v(j_min,1)+v(j_min+1,1) ) - x_min )* v(j_min,2); end;
if (size(v,1)>(j_max+1)) integral=integral+ ( x_max - 0.5*( v(j_max,1)+v(j_max+1,1) ) )* v(j_max,2); end;
if (j_min>1) 
...
% Do Right hand
j_right=j_start+1;
while(v(j_right,2)>y_right)&(j_right<j_max) j_right=j_right+1; end
...
% Do Left Hand
j_left=j_start-1;
while(v(j_left,2)>y_left)&(j_left>1) j_left=j_left-1; end
